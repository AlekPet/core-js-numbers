...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  // Find the length vectors
  const vecOneLen = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vecTwoLen = Math.sqrt(x2 ** 2 + y2 ** 2);

  // Find angle between two vectors
  return Math.acos(scalarProductVectors / (vecOneLen * vecTwoLen));
}
...
function isPrime(n) {
  ...
  const numSqrtRound = Math.round(Math.sqrt(n)); // find the square root of a number N, and rounded
  let result = false;

  for (let x = 2; x <= numSqrtRound; x += 1) {
    if ((n / x) % 1 === 0) return false;
    result = true;
  }
  return result;
}
...
function getCountOfOddNumbers(number) {
  let countOdd = 0;
  for (let x = Math.abs(number); x >= 0; x -= 1) if (x % 2 !== 0) countOdd += 1;
  return countOdd;
}
